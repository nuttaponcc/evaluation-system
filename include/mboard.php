...
$year=date("Y");
	    ...
	    echo $query2 = "SELECT * FROM  `staffinfo`  where staffinfo.staffmajor='ภาควิชาคณิตศาสตร์' or  staffinfo.staffid='133' or  staffinfo.staffid='331' ORDER BY staffname asc ";
	    $query2 = "SELECT * FROM  `staffinfo`  where 1ORDER BY staffname asc ";
...
//	echo "$query";
	...
	$result = mysql_query($query);
 	...
 	$query = "SELECT * FROM `workload`,`course`,`adjunct`  where adjunct.courseid=course.id and adjunct.workload=workload.id and  userid=$userid ORDER BY date asc";
	$result = mysql_query($query);
     while ($row = mysql_fetch_assoc($result)) {
	 $n=($row['t'])*$row['score']*$row['week']*$row['multiplier'];
	 $sum=$sum+$n;
   	...
   	$query = "SELECT * FROM `workload`,`is-advisor`  where  `is-advisor`.`workload`=`workload`.`id` and  `userid`=$userid ORDER BY date asc";
	//echo "$query";
	$result = mysql_query($query);
     while ($row = mysql_fetch_assoc($result)) {
	 		$n=$row['score']*$row['week']*$row['multiplier'];
