...
<div align="left">
  ...
  <?php 
     	...
     	$query = "SELECT * FROM `workload`,`course`,`coordinator`  where coordinator.courseid=course.id and coordinator.workload=workload.id and coordinator.id=$id and  userid='".$_SESSION[session_user_id]."' ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_array($result);
  ?>
  <p>1. ภาระงานด้านการสอน<br />
    1.3) ผู้ประสานงานรายวิชา </p>
  <form action="action/edit-coordinator.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
...
while ($row = mysql_fetch_array($result)) {
     if($row['id']==$row0['courseid'])        echo "<option selected=\"selected\" value=\"".$row['id']. "\" >".$row['code']." : ".$row['enname']." ".$row['tcredit']."  [".$row['term']."/".$row['ayear']."][กลุ่ม".$row['detail']."]</option>"; else  echo "<option value=\"".$row['id']. "\">".$row['code']." : ".$row['enname']." ".$row['tcredit']." [".$row['term']."/".$row['ayear']."][กลุ่ม".$row['detail']."] </option>";
	}	
	      echo "</select> ";
	  ?></td>
...
while ($row = mysql_fetch_assoc($result)) {
     if($row['id']==$row0['workolad'])     echo "<option selected=\"selected\" value=\"".$row['id']. "\" >".$row['detail'].":".$row['name']."(".$row['score'].") </option>"; else  echo "<option value=\"".$row['id']. "\">".$row['detail'].":".$row['name']."(".$row['score'].") </option>";
	}	
	      echo "</select> ";
	  ?></td>
