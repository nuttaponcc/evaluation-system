...
<div align="left">
  ...
  <?php 
     	...
     	$query = "SELECT * FROM `workload`,`course`,`adjunct`  where adjunct.courseid=course.id and adjunct.workload=workload.id and adjunct.id=$id and  userid='".$_SESSION[session_user_id]."' ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_array($result);
  ?>
  <p>1. ภาระงานด้านการสอน<br />
  1.11) การสอนเสริม (50 คน/กลุ่ม)</p>
  <form action="action/edit-adjunct.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
...
default:
	...
	$query = "SELECT * FROM `course` where   1  $cond    or  code  LIKE '0299%'  or  code  LIKE '12%'  or  code  LIKE '0506%'   ORDER BY `course`.`code` ASC ";
