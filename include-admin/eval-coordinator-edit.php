...
<div align="left">
  ...
  <?php 
     	...
     	$query = "SELECT * FROM `workload`,`course`,`coordinator`  where coordinator.courseid=course.id and coordinator.workload=workload.id and coordinator.id=$id and  userid=$userid ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_array($result);
  ?>
  <p>1. ภาระงานด้านการสอน<br />
    1.3) ผู้ประสานงานรายวิชา </p>
  <form action="action-admin/edit-coordinator.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
