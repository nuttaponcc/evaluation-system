...
$sql="UPDATE `service6` SET `name`='$name',`weight`='$workload',`multiplier`='$multiplier' WHERE  `service6`.`id`='$id' ";
$result = mysql_query($sql);

//echo $sql;
if(!empty($file)){
$ext=strtolower(end(explode('.', $file_name)));
$filename="service6".$id. "." .$ext;
copy($file,"../files/$filename");

$sql="update  `service6` set  filename='$filename' where id='$id' ";
$result = mysql_query($sql);
}
$userid=$_GET['userid'];
