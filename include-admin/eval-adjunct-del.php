...
<div align="left">
  ...
  <form action="action-admin/del-adjunct.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
  <table border="0" cellpadding="5" bgcolor="#FFFFFF">
	...
	$query = "SELECT * FROM `course` where  id=".$row0[17] ;
	$result = mysql_query($query);
...
echo $row['code']." : ".$row['enname']." ".$row['tcredit'];
    ...
    <tr>
      ...
      <td align="left" valign="top" bgcolor="#FFDDFF"><?
	$query = "SELECT * FROM `workload` where category=12 and  id=".$row0[19];
	$result = mysql_query($query);
while ($row = mysql_fetch_assoc($result)) {
