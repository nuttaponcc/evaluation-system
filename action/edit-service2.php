...
$sql="UPDATE`service2` SET `name`='$name',`weight`='$workload',`multiplier`='$multiplier' WHERE  `service2`.`id`='$id' ";
$result = mysql_query($sql);

//echo $sql;
if(!empty($file)){
$ext=strtolower(end(explode('.', $file_name)));
$filename="service2".$id. "." .$ext;
copy($file,"../files/$filename");

$sql="update  `service2` set  filename='$filename' where id='$id' ";
$result = mysql_query($sql);
}
