...
default:
	...
	$query = "SELECT * FROM `course` where p>0 $cond    or  code  LIKE '0299%'  or  code  LIKE '12%'   or  code  LIKE '0506%'   ORDER BY `course`.`code` ASC ";

	$result = mysql_query($query);
	       echo "<select name=\"course\">";
while ($row = mysql_fetch_assoc($result)) {
     if($row['id']==$row0[17])       echo "<option selected=\"selected\" value=\"".$row['id']. "\" >".$row['code']." : ".$row['enname']." ".$row['tcredit']." </option>"; else  echo "<option value=\"".$row['id']. "\">".$row['code']." : ".$row['enname']." ".$row['tcredit']." </option>";
	}	
	      echo "</select> ";
	  ?></td>
...
?>
      ...
      <td align="left" valign="top" bgcolor="#FFDDFF"><div id="textmultiplier">
        <script language="JavaScript" type="text/javascript">
				javascript:changediv('<? echo $row0[17]?>','multiplier')
	  </script>
      </div></td>
    </tr>
    <tr>
