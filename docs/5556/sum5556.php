...
<title>สรุปโครงการ วงรอบการประเมิน วงรอบที่ 2 (1 มีนาคม พ.ศ. 2556 ถึง 31 สิงหาคม พ.ศ. 2556)</title>
</head>

<body>
<p align="center"><strong>สรุปโครงการ <br />
วงรอบการประเมิน  วงรอบที่ 2 (1 มีนาคม พ.ศ. 2556 ถึง 31 สิงหาคม พ.ศ. 2556) </strong></p>
...
<div align="center">
  <table border="0" cellpadding="0" cellspacing="10">
    <tr>
      <td valign="top"><table width="240" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFCCCC">
        <tr>
...
<tr><td>รหัสโครงการ</td><td  align=\"left\"   ><input name="idcode" type="text" value="<? echo $idcode  ?>" /> </td></tr>				
      ...
      <td valign="top"><table border="1" cellspacing="0" cellpadding="0" width="100%">
        <tr>
          <td width="109" valign="top" bgcolor="#66CCCC"><? 
...
if($idcode!="")
{
	 ...
	 $sql=" SELECT * FROM `project`  where  `status`>0   $str_agencies  $str_subj   $str_idcode  ORDER BY `project`.`agenid` ASC";
//echo $sql;	 
...
if($i<0) $str="bgcolor=\"#fdfdfd\"";  else  $str="bgcolor=\"#ffffff\""; 
$i=$i*-1;
...
if ( $row['status']>0 )$tstat="<font color=blue>". $statarr[$row['status']] ."</font>"; else $tstat="<font color=red>". $statarr[$row['status']] ."(จะไม่แสดง)</font>";
	 ...
	 while($row1=mysql_fetch_array($result1))
	 {
        $f1=$f1."&#8226; <a   target=\"_blank\"  href=\"../../../sciproject/". $row1['attlocation']."/".$row1['attfile']."\" >".$row1['attfile']."</a><br>";
	 }
	 ...
	 while($row1=mysql_fetch_array($result1))
	 {
        $f3=$f3." <a   target=\"_blank\"  href=\"../../../sciproject/". $row1['attlocation']."/".$row1['attfile']."\" >".$row1['attfile']."</a><br>";
	     ...
	     echo "<tr><td $str> ".  $agen[$row['agenid']] ." </td><td $str>". $row['idcode']  ."</td><td $str> ". $row['prosubj']  ."   </br><font color=\"#008800\"> ".  $row['prosuben'] ." </font></td><td $str> ".   $categoryarr[$row['procategory']] ." </td><td $str>  ".$f1 . "  </td><td align=center  $str>  ". $f2 ." </td><td $str>  ".$f3.  " </td><td $str>  ".$tstat  .  " </td></tr>";
