...
<div align="left">
  <p><strong>แก้ไข</strong>
    <?php 
     	$query = "SELECT * FROM `workload`,`internships`  where  internships.workload=workload.id and  internships.id=$id  and  userid=$userid ORDER BY date asc";
  ...
  <p>1. ภาระงานด้านการสอน<br />
    1.4) การควบคุมการฝึกงาน/การฝึกสอน/สหกิจ</p>
  <form action="action-admin/edit-internships.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
  <table border="0" cellpadding="5" bgcolor="#FFFFFF">
    <tr>
      <td align="left" valign="top" bgcolor="#FFDDFF"><input name="id" type="hidden" value="<? echo $id ?>" />
        ชื่อ-นามสกุลนิสิต</td>
      <td align="left" valign="top" bgcolor="#FFDDFF"> <input name="name" type="text" id="name" value="<? echo $row0[15] ?>" /></td>
...
?>
	...
	$query = "SELECT * FROM `workload` where category=5 ";
	$result = mysql_query($query);
...
ตัวคูณ=4 ผลงานของนิสิตในที่ปรึกษาได้รับการเผยแพร่ รูปแบบใดรูปแบบหนึ่ง <br />
ตัวคูณ=5 ผลงานของนิสิตในที่ปรึกษาได้รับรางวัล ยกย่องเชิดชูเกียรติ </span></td>
    </tr>
    <!--tr>
