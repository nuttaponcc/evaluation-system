...
<div align="left">
  ...
  <?php 
     	$query = "SELECT * FROM `workload`,`undergraduate-advisor`  where  `undergraduate-advisor`.`workload`=workload.id and  `undergraduate-advisor`.id=$id  and  userid=$userid ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_row($result);
  ?>
  <p>1. ภาระงานด้านการสอน<br />
    1.5) ที่ปรึกษาโครงการปริญญานิพนธ์ ระดับปริญญาตรี</p>
  <form action="action-admin/edit-ug-advisor.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
...
?>
	...
	$query = "SELECT * FROM `workload` where category=6 ";
	$result = mysql_query($query);
