...
<div align="left"><p><strong>แก้ไข</strong>
    <?php 
     	$query = "SELECT * FROM `workload`,`service5`  where  `service5`.`weight`=workload.id and `service5`.id=$id  and  userid=$userid ORDER BY date asc";
  ...
  <form action="action-admin/edit-service5.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
    <table border="0" cellpadding="5" bgcolor="#FFFFFF">
    <tr>
      <td align="left" valign="top" bgcolor="#BBFFFF"><input name="id" type="hidden" value="<? echo $id ?>" />
        ชื่อโครงการ</td>
      <td align="left" valign="top" bgcolor="#BBFFFF"><input name="name" type="text" id="name" value="<? echo $row0['name']; ?>" /></td>
...
while ($row = mysql_fetch_assoc($result)) {
     if($row['id']==$row0['weight'])       echo "<option selected=\"selected\" value=\"".$row['id']. "\" >".$row['name']." (".$row['score'].")</option>"; else  echo "<option value=\"".$row['id']. "\">".$row['name']." (".$row['score'].")</option>";;
