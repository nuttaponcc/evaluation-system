...
$sql="UPDATE`service3` SET `name`='$name',`amount`='$amount' ,`multiplier`='$multiplier'   WHERE  `service3`.`id`='$id' ";
$result = mysql_query($sql);

//echo $sql;
if(!empty($file)){
$ext=strtolower(end(explode('.', $file_name)));
$filename="service3".$id. "." .$ext;
copy($file,"../files/$filename");

$sql="update  `service3` set  filename='$filename' where id='$id' ";
$result = mysql_query($sql);
}
$userid=$_GET['userid'];
