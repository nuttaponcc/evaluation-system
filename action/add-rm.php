...
$sql="INSERT INTO rm VALUES(NULL,'$session_user_id','$course','$week','$workload','$multiplier',0,'','$date_today','$year')";
$result = mysql_query($sql);
if(!empty($file)){
//echo $sql;
$ext=strtolower(end(explode('.', $file_name)));

$sql="select max(id) from  rm";
$result = mysql_query($sql);
$r=mysql_fetch_array($result);
$id_max=$r[0];

$filename="rm".$id_max. "." .$ext;
copy($file,"../files/$filename");

$sql="update  rm set  filename='$filename' where id='$id_max' ";
$result = mysql_query($sql);
}
