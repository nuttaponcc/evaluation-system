...
<div align="left">
  <p><strong>แก้ไข</strong><?php 
     	$query = "SELECT * FROM `workload`,`seniorproject`  where  `seniorproject`.`workload`=workload.id and  `seniorproject`.id=$id  and  userid='".$_SESSION[session_user_id]."' ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_row($result);
  ?>
  </p>
  <p>1. ภาระงานด้านการสอน<br />
  1.10) กรรมการสอบ Senior Project/วิทยานิพนธ์</p>
  <form action="action/edit-seniorproject.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
