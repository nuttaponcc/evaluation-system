...
$query = "SELECT * FROM `workload`,`course`,`adjunct`  where adjunct.courseid=course.id and adjunct.workload=workload.id and adjunct.id=$id and  userid=$userid ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_array($result);
  ?>
  <p>1. ภาระงานด้านการสอน<br />
1.11) การสอนเสริม (50 คน/กลุ่ม)</p>
  <form action="action-admin/edit-adjunct.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
  <table border="0" cellpadding="5" bgcolor="#FFFFFF">
	...
	$query = "SELECT * FROM `course` where   1 ORDER BY `course`.`code` ASC ";
	$result = mysql_query($query);
     echo "<select name=\"course\">";
while ($row = mysql_fetch_array($result)) {
     if($row['id']==$row0['courseid'])        echo "<option selected=\"selected\" value=\"".$row['id']. "\" >".$row['code']." : ".$row['enname']." ".$row['tcredit']."  [".$row['term']."/".$row['ayear']."]</option>"; else  echo "<option value=\"".$row['id']. "\">".$row['code']." : ".$row['enname']." ".$row['tcredit']."  [".$row['term']."/".$row['ayear']."]</option>";
	}	
	      echo "</select> ";
	  ?></td>
...
?>
      ...
      <td align="left" valign="top" bgcolor="#FFDDFF">ตัวคูณ=3 มีรายละเอียดของรายวิชา (มคอ.3) และ รายละเอียดของประสบการณ์ (ถ้ามี) (มคอ.4) <br />
ตัวคูณ=4 มี มคอ.3 และ มคอ.4 (ถ้ามี) และส่งผลการเรียนภายในระยะเวลาที่คณะกำหนด<br />
ตัวคูณ=5 มีการจัดการเรียนการสอนที่พัฒนาจากานวิจัย หรือมีการจัดการความรู้เพื่อพัฒนาการเรียนการสอน หรือมีการนำประสบการณ์จากการให้บริการวิชาการสู่สังคมหรือชุมชนมาพัฒนาการจัดการเรียน </td>
    </tr>
    <tr>
