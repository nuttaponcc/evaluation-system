...
<div align="left">
  <p><strong>แก้ไข</strong> 
    <?php 
     	$query = "SELECT * FROM `workload`,`other`  where  `other`.`weight`=`workload`.`id` and  `workload`.`category`=$other  and `other`.`id`=$id and   userid=$userid ORDER BY date asc";		
		//echo  $query;
...
?>		
  ...
  <form action="action-admin/edit-other.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
...
ตัวคูณ= 5  มีการพัฒนาและปรับปรุงหลักสูตรอย่างต่อเนื่อง หรือ กลุ่ม ISO<br />
    ...
    <tr>
      ...
      <td align="left" valign="top" bgcolor="#F3F2F7"><input name="file7" type="file" id="file7" />
          <? echo "<a href=\"files/".$row0['filename7']."\" >".$row0['filename7']."</a> " ?></td>
      <td align="left" valign="top" bgcolor="#F3F2F7">นำหลักฐานทั้งหมดบีบอัดมาเป็นไฟล์เดียว ( ชนิด ZIP หรือ RAR ) </td>
    </tr>
	<? } ?>
  </table>
