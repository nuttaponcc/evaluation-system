import { Suspense } from 'react';
import Link from 'next/link';
import { Plus, Pencil } from 'lucide-react';
import { prisma } from '@/lib/prisma';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { formatDateTime } from '@/lib/utils';
import { DeleteButton } from '@/components/admin/delete-button';
import { ToggleActiveButton } from '@/components/admin/toggle-active-button';

interface AdminActivitiesPageProps {
  searchParams: Promise<{
    page?: string;
  }>;
}

async function ActivitiesList({ searchParams }: { searchParams: AdminActivitiesPageProps['searchParams'] }) {
  const params = await searchParams;
  const page = parseInt(params.page || '1');
  const limit = 20;

  const [activities, total] = await Promise.all([
    prisma.activity.findMany({
      orderBy: { createdAt: 'desc' },
      skip: (page - 1) * limit,
      take: limit,
    }),
    prisma.activity.count(),
  ]);

  return (
    <div className="space-y-4">
      <div className="rounded-md border">
        <table className="w-full">
          <thead>
            <tr className="border-b bg-muted/50">
              <th className="p-4 text-left font-medium">รหัส</th>
              <th className="p-4 text-left font-medium">ชื่อกิจกรรม</th>
              <th className="p-4 text-left font-medium">หมวดหมู่</th>
              <th className="p-4 text-left font-medium">วันที่จัด</th>
              <th className="p-4 text-left font-medium">สถานที่</th>
              <th className="p-4 text-left font-medium">สถานะ</th>
              <th className="p-4 text-right font-medium">จัดการ</th>
            </tr>
          </thead>
          <tbody>
            {activities.map((activity) => (
              <tr key={activity.id} className="border-b hover:bg-muted/50">
                <td className="p-4 text-sm">{activity.activityId}</td>
                <td className="p-4 font-medium">{activity.title}</td>
                <td className="p-4">
                  <Badge variant="secondary">{activity.category}</Badge>
                </td>
                <td className="p-4 text-sm">{formatDateTime(activity.startDatetime)}</td>
                <td className="p-4 text-sm">{activity.location}</td>
                <td className="p-4">
                  <Badge variant={activity.isActive ? 'success' : 'secondary'}>
                    {activity.isActive ? 'เปิดใช้งาน' : 'ปิดใช้งาน'}
                  </Badge>
                </td>
                <td className="p-4">
                  <div className="flex items-center justify-end gap-2">
                    <ToggleActiveButton
                      id={activity.id}
                      isActive={activity.isActive}
                      endpoint="/api/activities"
                    />
                    <Link href={`/admin/activities/${activity.id}/edit`}>
                      <Button variant="ghost" size="icon">
                        <Pencil className="h-4 w-4" />
                      </Button>
                    </Link>
                    <DeleteButton id={activity.id} endpoint="/api/activities" />
                  </div>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <div className="text-sm text-muted-foreground">
        แสดง {activities.length} จาก {total} รายการ
      </div>
    </div>
  );
}

export default async function AdminActivitiesPage({ searchParams }: AdminActivitiesPageProps) {
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">จัดการกิจกรรม</h1>
          <p className="text-muted-foreground mt-2">
            จัดการกิจกรรมทั้งหมด
          </p>
        </div>
        <Link href="/admin/activities/new">
          <Button>
            <Plus className="h-4 w-4 mr-2" />
            เพิ่มกิจกรรมใหม่
          </Button>
        </Link>
      </div>

      <Suspense fallback={<div className="text-center py-12">กำลังโหลด...</div>}>
        <ActivitiesList searchParams={searchParams} />
      </Suspense>
    </div>
  );
}
