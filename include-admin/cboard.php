...
$year=date("Y");
	    ...
	    $query2 = "SELECT * FROM  `staffinfo`  where staffinfo.staffmajor='ภาควิชาเคมี'  or  staffinfo.staffid='536'   or  staffinfo.staffid='194'  or  staffinfo.staffid='447'    ORDER BY staffname asc ";//or  staffinfo.staffid='205'  
...
$s33=0;
	...
	$result = mysql_query($query);
	 ...
	 $t="$t+$n";
  	...
  	$query = "SELECT * FROM `workload`,`service1`  where `weight`=`workload`.`id` and  userid='".$row2['staffid']."' ORDER BY date asc";
	//echo "$query";
	$result = mysql_query($query);
     while ($row = mysql_fetch_assoc($result)) {
