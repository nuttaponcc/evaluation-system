...
case  'multiplier':
		switch(text)
			{
				case "1":
							  document.getElementById("textmultiplier").innerHTML = "ตัวคูณ=1 ";
				  break;
				case "2":
...
<div align="left">
  <p><strong>แก้ไข</strong><?php 
     	$query = "SELECT * FROM `workload`,`seniorproject`  where  `seniorproject`.`workload`=workload.id and  `seniorproject`.id=$id  and  userid=$userid ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_row($result);
  ?>
  </p>
  <p>1. ภาระงานด้านการสอน<br />
  1.10) กรรมการสอบ Senior Project/วิทยานิพนธ์/คณะกรรมการสอบสัมมนา/กรรมการคุมสอบ </p>
  <form action="action-admin/edit-seniorproject.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
...
?>
	...
	$query = "SELECT * FROM `workload` where category=15 ";
	$result = mysql_query($query);
