...
<div align="center"><br>
<input name="btnExport" type="button" value="Export CSV (Excel)" onClick="JavaScript:window.location='mexport.php';">
</div>
...
$year=date("Y");
	    ...
	    $query2 = "SELECT * FROM  `staffinfo`  where   1 ORDER BY staffinfo.staffmajor asc,staffname asc";
		$result2 = mysql_query($query2);
		
		echo "<table border=0 cellpadding=10 ><tr><td align=center>";
		echo "<strong>ตรวจสอบข้อมูลของบุคลากรคณะวิทยาศาสตร์</strong><br><br>";
		echo "<table border=1 cellpadding=5 >";
		$i=1;
// if($Mrow ['b']==2)
// {
  $str ="<td align=cenetr>คะแนนดิบด้านการสอน</td><td>score</td><td>level</td><td>score</td><td>level</td><td align=center>คะแนนดิบด้านวิจัย</td><td>score</td><td>level</td><td align=center>คะแนนดิบด้านการบริการวิชาการ(3.1-3.4)(3.5-3.6)</td><td>score</td><td>level</td><td align=center>คะแนนดิบด้านทำนุบำรุงศิลปะและวัฒนธรรม</td><td>score</td><td>level</td><td align=center>คะแนนดิบด้านช่วยการบริหาร<br>จัดการและอื่น ๆ(5.1)
(5.2-5.5)</td><td>score</td><td>level</td><td>ผลรวมของค่าคะแนน</td><td>%เต็ม1</td><td>เต็ม70%</td><td>เต็ม30%</td><td>เต็ม100%</td>";
//  }else{
//  $str="";
//  }
  
  echo "<tr><td align=center>ลำดับที่</td><td align=center> ชื่อ </td><td align=center>หน่วยงาน</td></tr>";//$str
...
*/
	...
	$result = mysql_query($query);
     while ($row = mysql_fetch_assoc($result)) {
//	 		$n=$row['multiplier'];
	 		$n=$row['multiplier']*.3*$row['percent']/100;			
	        $sum2=$sum2+$n;
}
 ...
 $sum31=0;
	...
	$result = mysql_query($query);
	 ...
	 $t="$t+$n";
  	...
  	$query = "SELECT * FROM `service7`  where  userid='".$row2['staffid']."' ORDER BY date asc";
	//echo "$query";
	$result = mysql_query($query);
     while ($row = mysql_fetch_assoc($result)) {
...
$sum5=0;
	...
	$result = mysql_query($query);
     while ($row = mysql_fetch_assoc($result)) {
	 		$n=$row['score']*$row['multiplier']*0.05;
     if($row['id']==50)$txt="(".$row['name'].")"; else $txt="";
	 $sum51=$sum51+$n;
	 }	
/*	
...
$level[$r2['beh26']]++;
$beh=0;
$beh=round(($level[5]*3+$level[4]*2+$level[3])/33,2)*30 ;
}

$total=$p2+$beh;
 $str ="<td align=center><strong>$sum</strong> </td><td><font color=green>$score1</font></td><td><font color=blue>$grade1</font></td><td align=center><strong>$sum2</strong></td><td><font color=green>$score2</font></td><td><font color=blue>$grade2</font></td><td align=center><strong>($sum3)($sum31)</strong></td><td><font color=green>$score3</font></td><td><font color=blue>$grade3</font></td><td align=center><strong>$sum4</strong></td><td><font color=green>$score4</font></td><td><font color=blue>$grade4</font></td><td align=center><strong>($sum5)($sum51)</strong></td><td><font color=green>$score5</font></td><td><font color=blue>$grade5</font></td><td>$Tlevel</td><td>$p1</td><td>$p2</td><td>$beh</td><td>$total</td>";
...
/***********จบคะแนนดิบ************/
		        ...
		        if ($total==0) echo "<tr><td align=center>".$i++."</td><td >".$row2 ['staffname']." </td><td > ".$row2 ['staffmajor']. " </td>$str </td></tr>";
		       //if ($total>0) echo "<tr><td align=center>".$i++."</td><td >".$row2 ['staffname']." </td><td > ".$row2 ['staffmajor']. " </td>$str </td></tr>";
		 }
		 echo"</table>";
		 echo"</td></tr></table>";
