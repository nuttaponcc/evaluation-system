...
<div align="left">
  <p><strong>ลบ</strong><?php 
     	...
     	$query = "SELECT * FROM `workload`,`course`,`lectures`  where lectures.courseid=course.id and lectures.workload=workload.id and lectures.id=$id and  userid=$userid ORDER BY date asc";
  ...
  <form action="action-admin/del-lectures.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
...
echo $row['code']." : ".$row['enname']." ".$row['tcredit'];
    ...
    <tr>
      ...
      <td align="left" valign="top" bgcolor="#FFDDFF"><?
	$query = "SELECT * FROM `workload` where category=1 and  id=".$row0[19];
	$result = mysql_query($query);
while ($row = mysql_fetch_assoc($result)) {
