...
<div align="left"><p><strong>แก้ไข</strong>
    <?php 
     	$query = "SELECT * FROM `workload`,`service2`  where  `service2`.`weight`=workload.id and `service2`.id=$id  and  userid=$userid ORDER BY date asc";
  ...
  <form action="action-admin/edit-service2.php?<? echo "userid=$userid" ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
...
while ($row = mysql_fetch_assoc($result)) {
     if($row['id']==$row0[13])       echo "<option selected=\"selected\" value=\"".$row['id']. "\" >".$row['detail'].":".$row['name']." </option>"; else  echo "<option value=\"".$row['id']. "\">".$row['detail'].":".$row['name']." </option>";;
