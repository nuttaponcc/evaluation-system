...
define(‘FPDF_FONTPATH’,'font/');
		...
		$query = "SELECT * FROM `weight`,`wselect` where  weight.id=wselect.wtid and wselect.userid= '$_SESSION[session_user_id]' ";
		$result = mysql_query($query);
     	if($row = mysql_fetch_assoc($result));if($row['status'])$percent=50; else $percent=70;

		$query = "SELECT * FROM `stscore` where id_st= '$_SESSION[session_user_id]' ";
		$result = mysql_query($query);
        $row = mysql_fetch_assoc($result);
		$sc70=$row[per2]/$percent; 
		$sc30=$row[per3]/(30); 
; 
...
$pdf->SetXY(60,8+$h*6);
//$pdf->Cell(15,10,'	 รอบที่ 1 วันที่ 1 ตุลาคม พ.ศ.2555  – 31 มีนาคม พ.ศ.2556',0,1,'L');
$pdf->SetXY(60,8+$h*7);
...
$pdf->SetXY(60,8+$h*13);
$pdf->Cell(15,10,'สังกัด  :  คณะวิศวกรรมศาสตร์ มหาวิทยาลัยมหาสารคาม ',0,1,'L');
...
$pdf->SetXY(145,4+$h*3);
//$pdf->Cell(10,10,' สังกัด ...คณะวิศวกรรมศาสตร์ มหาวิทยาลัยมหาสารคาม....       ระยะเวลาที่ดำรงตำแหน่ง ...'.$r['ptime'].'...   แผนพัฒนารายบุคคลประจำปี    ....2555....',0,1,'C');
...
if($smajor=='ภาควิชาฟิสิกส์'||$smajor=='ภาควิชาชีววิทยา'||$_SESSION[session_user_id]==240||$_SESSION[session_user_id]==240) $pdf->MultiCell(78,5,$sc70,0,'L'); 
$pdf->SetXY(110,10+$h*5);
if($smajor=='ภาควิชาฟิสิกส์'||$smajor=='ภาควิชาชีววิทยา'||$_SESSION[session_user_id]==240||$_SESSION[session_user_id]==500) $pdf->MultiCell(78,5,'                           '.$percent.'                     '.$sc7.'',0,'L'); 
$pdf->SetXY(21,10+$h*7);
$pdf->MultiCell(78,5,'องค์ประกอบที่ 2 พฤติกรรมการปฏิบัติราชการ',0,'L');
$pdf->SetXY(110,10+$h*7);
if(($smajor=='ภาควิชาฟิสิกส์'||$smajor=='ภาควิชาชีววิทยา')&&($sc30>0)) $pdf->MultiCell(78,5,$sc30,0,'L');
$pdf->SetXY(110,10+$h*7);
if(($smajor=='ภาควิชาฟิสิกส์'||$smajor=='ภาควิชาชีววิทยา')&&($sc30>0)) $pdf->MultiCell(78,5,'                           '. (100-$percent) .'                     '.$sc3.'',0,'L');

$pdf->SetXY(21,10+$h*9);
$pdf->MultiCell(78,5,'องค์ประกอบอื่นๆ (ถ้ามี)',0,'L');
//if($smajor!='ภาควิชาฟิสิกส์')$pdf->MultiCell(78,5,$sc30,0,'L');

$pdf->SetXY(120,8+$h*11);
$pdf->MultiCell(78,5,'รวม              100                 '.($sc7+$sc3),0,'L');
...
while($r2 = mysql_fetch_assoc($result))
{
	...
	$s=$r2['subj'];
	$pdf->SetXY(10,50+5*$h*$i);
	$pdf->MultiCell( 70 , 4 , $s , 0, 'L', 0);        
	$s=$r2['method'];
	$pdf->SetXY(80,50+5*$h*$i);
	$pdf->MultiCell( 78 , 4 , $s);        
	$s=$r2['ptime'];
	$pdf->SetXY(160,50+5*$h*$i);
	$pdf->MultiCell( 38 , 4 , $s );        
	$x1=0;
...
$pdf->Line($x1+10,$y2+$i*$h2, $x2-10, $y2+$i*$h2);
$pdf->Line($x1+10,$y1+$i*$h2, $x1+10, $y2+$i*$h2);
$pdf->Line($x2-10,$y1+$i*$h2, $x2-10, $y2+$i*$h2);
$pdf->Line(80,$y1+$i*$h2, 80, $y2+$i*$h2);
$pdf->Line(160,$y1+$i*$h2, 160, $y2+$i*$h2);

	$i++;

	if(($i>=8)||(($i+($page-1)*8)==$num_rows))
	{
	   $i=0;
      $pdf->AddPage();
	}
