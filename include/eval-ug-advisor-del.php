...
<div align="left">
  ...
  <?php 
     	$query = "SELECT * FROM `workload`,`undergraduate-advisor`  where  `undergraduate-advisor`.`workload`=workload.id and  `undergraduate-advisor`.id=$id  and  userid='".$_SESSION[session_user_id]."' ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_row($result);
  ?>
  <p>1. ภาระงานด้านการสอน<br />
    1.5) ที่ปรึกษาโครงการปริญญานิพนธ์ ระดับปริญญาตรี</p>
  <form action="action/del-ug-advisor.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
  <table width="238" border="0" cellpadding="5" bgcolor="#FFFFFF">
    <tr>
      <td align="left" valign="top" bgcolor="#FFDDFF"><input name="id" type="hidden" value="<? echo $id ?>" />
        ชื่อ-นามสกุลนิสิตและชื่อโครงการ</td>
      <td width="20" align="left" valign="top" bgcolor="#FFDDFF"><? echo $row0[14]; ?></td>
      <td width="1" align="left" valign="top" bgcolor="#FFDDFF">&nbsp;</td>
    </tr>
    <tr>
      <td width="179" align="left" valign="top" bgcolor="#FFDDFF">จำนวนสัปดาห์ <span class="style1">*</span></td>
...
?>
    ...
    <tr>
      ...
      <td align="left" valign="top" bgcolor="#FFDDFF"><?
	$query = "SELECT * FROM `workload` where category=6  and  id=".$row0[8];
	$result = mysql_query($query);
