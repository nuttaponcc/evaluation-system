...
<div align="left">
  <p><strong>ลบ</strong><?php 
     	$query = "SELECT * FROM `workload`,`academic-advisor`  where  `academic-advisor`.`workload`=workload.id and  `academic-advisor`.id=$id  and  userid='".$_SESSION[session_user_id]."' ORDER BY date asc";
		$result = mysql_query($query);
		$row0 = mysql_fetch_row($result);
  ?>
  </p>
  <p>1. ภาระงานด้านการสอน<br />
    1.8) อาจารย์ที่ปรึกษาทางด้านวิชาการอื่น</p>
  <form action="action/del-academic-advisor.php" method="post" enctype="multipart/form-data" name="form1" id="form1">
...
?></td>
    ...
    <tr>
      ...
      <td align="left" valign="top" bgcolor="#FFDDFF"><?
	$query = "SELECT * FROM `workload` where category=10 and id=".$row0[8];
	$result = mysql_query($query);
